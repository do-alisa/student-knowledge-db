# Name of application: StudentKnowledgeDB

<img width="1504" height="1022" alt="Screenshot 2025-12-05 165342" src="https://github.com/user-attachments/assets/7d2d4b5b-d7f3-4714-8e99-6421240c079a" />
<img width="1354" height="947" alt="image" src="https://github.com/user-attachments/assets/fa9019c6-7b43-4ec3-bd3f-099f1c1dec00" />

**Version:** 0.9
## who did what:
1. **Alisa Do**  – Implemented Reports page and main functionality.
2. **Jiang Du**  – Added the default students and associated comments.
3. **Jamie O'Neill** - Added popup window for expanding comments, fixed small issues in Reports page
4. **Cam Hung Phuong**  – Tested the new features and fixed bugs in the Reports and Comments functions.







**Version:** 0.8
## who did what:
1. **Alisa Do**  – Fixed and refined the Add Comments feature; ensured exactly 5 Student Profiles were populated.
2. **Jiang Du**  – Tested the Add Comments functionality, verified data persistence, and cleaned up the project folder.
3. **Jamie O'Neill**- Modified and optimized the UI layout, buttons, and improved the display for the Existing Profiles page.
4. **Cam Hung Phuong**  – Implemented the Add Comments feature and fixed previous version bugs related to Comments (including issue with exceeding 5 student profiles).









**Version:** 0.7
## who did what: 
1. **Alisa Do**  – Assisted with Brainstorming, Testing, and Debugging of the Edit Student Profile page
2. **Jiang Du**  – Adding edit Students Profiles pages and store data permanently
3. **Jamie O'Neill**- Changed profiles detail page to match language used in the rest of the Application, updated UI, fixed minor errors in edit Student Profile
4. **Cam Hung Phuong**  – Assisted testing and improving the edit Students Profiles pages.





**Version:** 0.6

## who did what:
1. **Alisa Do** – Updated UI labels in Search Students Profiles page for clarity and consistency, ensured consistency across all pages
2. **Jiang Du** – Added default data for three programming languages and five student profiles, and created a new Search Page, implemented a Delete Feature for removing existing student records.
3. **Jamie O'Neill** - UI Changes to all pages to improve application-wide consistancy. Minor changes to the Search page.
4. **Cam Hung Phuong** – Assisted with testing and debugging the Search and Delete features

---

**Version:** 0.5

## who did what:
1. **Alisa Do** – Create Student Profile page (required fields, radio buttons, comments/journal, whitelist/blacklist), added toggle for All Student Profiles table
2. **Jiang Du** – Implemented student profiles page (required fields, multi-select Programming Languages, All Student Profiles table) and saving information into CSV file.
3. **Jamie O'Neill** - Updated UI on CreateStudentProfiles for consistancy with rest of the application, added warning messages to CreateStudentProfiles 
4. **Cam Hung Phuong** – Tested software

---

**Version:** 0.3

## who did what:
1. Alisa Do - Fixed TextField
2. Jiang Du - Tested software
3. Jamie O'Neil - Updated UI
4. Cam Hung Phuong - Added feature showing stored Programming Languages info in a tabular format, sorted alphabetically ascending

---

**Version:** 0.2

## who did what:
1. Alisa Do - Home Page, navigation to Define Programming Languages page, project manager
2. Jiang Du - Define Programming Languages page: Implemented TextArea input, Back, Cancel, and Submit actions.
3. Jamie O'Neill - Updated UI of Home Page/Define Programming Languages
3. Cam Hung Phuong - Tested software
