...
public class StudentProfilesController {
    ...
    private static final Path LANG_FILE = Paths.get(
            System.getProperty("user.home"), ".studentknowledgedb2", "languages.txt");
    ...
    private static final String FACULTY_TAG = "FACULTY_NOTE";                  // keep your tag convention
    ...
    private void initialize() {
        ...
        commentsCol.setCellValueFactory(c ->
                new javafx.beans.property.SimpleStringProperty(latestCommentFor(c.getValue())));
    ...
    // Actions
    @FXML
    private void onSubmitProfile() throws IOException {
    ...
    // return the newest normal comment (ignores FACULTY_NOTE|... entries)
    private String latestCommentFor(StudentProfile p) {
        ...
        return p.getComments().stream()
                ...
                .max(Comparator.comparing(this::commentDateSafe)
                        .thenComparing(Comparator.naturalOrder()))
                .orElse(""); // your comments already look like "yyyy-MM-dd — text"
    }

    // parse leading yyyy-MM-dd from a string like "yyyy-MM-dd — text"
    private LocalDate commentDateSafe(String c) {
        try { return LocalDate.parse(c.substring(0, 10), DATE_ONLY); }
        catch (Exception e) { return LocalDate.MIN; } // unknown format -> sort last
    }
