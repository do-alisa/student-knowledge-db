...
public final class DefaultThreeLangAndFiveStudents {
    ...
    public static void run() {
        ...
        // Seed languages if missing (optional — remove this if you want to preserve custom edits)
        if (!Files.exists(LANG_TXT)) {
            resetLanguagesToExactly3();
        }

        // IMPORTANT: only seed profiles if the file doesn't exist
    ...
    private static void resetProfilesToExactly5() {
        // DO NOT delete if it exists; this method is only called when it doesn't.
        ProfileStore store = new ProfileStore();
        StudentProfile p;

        // ... (same 5 students as you already have) ...
        // A
